...
class PromptGeneratorOrchestrator:
    ...
    def read_override_fields(self) -> Dict[str, Any]:
        """Read organization-specific override fields from configs.config (if present)."""
        ...
        if overrides:
            ...
            print("ℹ️  No field overrides provided (OVERRIDE_FIELDS not set)")
        return overrides
...
"""
            ...
            # Include Field Overrides section if provided
            overrides = prompt_data.get("overrides", {}) or {}
            if overrides:
                prompt_data_section += f"""

## 🔧 Field Overrides (USE EXACT VALUES)

The following field values MUST be used exactly as specified in every generated test case. If a field appears below, set it to this value regardless of samples or defaults.
...
"""
            ...
            # Prepare schema for prompt display, applying overrides when present
            schema_for_prompt = deepcopy(
                prompt_data.get("derived_format", {}).get("schema", {})
            )
            if overrides:
                for k, v in overrides.items():
                    # Apply only to known fields to avoid confusing the schema
                    if isinstance(schema_for_prompt, dict):
                        schema_for_prompt[k] = v
...
Derive the format from recent generated cases and required custom fields. Only include fields that are NOT auto-generated by the system.
            ...
            Based on `@target/generated_test_cases.json` and required custom fields, each test case MUST follow this JSON structure (with overrides applied if present):
```json
...
"""
            ...
            if sample_test_cases:
                ...
                # Build sample block and apply overrides
                sample_block = {
                    ...
                    "custom_preconds": sample.get(
                        "custom_preconds", "Sample preconditions"
                    ),
                    "custom_steps_separated": [
                        {
                ...
                prompt_data_section += (
                    "```json\n" + json.dumps(sample_block, indent=2) + "\n```\n"
                )
            else:
...
6. **Save the output** to: `target/generated_test_cases.json`
   - If `target/generated_test_cases.json` already exists, first rename the existing file to `target/generated_test_cases_old.json`, then create a fresh new `target/generated_test_cases.json` with the newly generated test cases.
...
5. **Final output structure**:
   - Save ALL generated test cases to: `target/generated_test_cases.json`
     - If this file already exists, first rename the existing file to `target/generated_test_cases_old.json`, then create a fresh new `target/generated_test_cases.json`.
   ...
   - If no duplicates are found, DO NOT save the `target/possible_duplicate_cases.json` file.
...
"""
    ...
    def run_workflow(self):
        ...
        # Step 3.6: Read organization-specific override fields
        overrides = self.read_override_fields()
